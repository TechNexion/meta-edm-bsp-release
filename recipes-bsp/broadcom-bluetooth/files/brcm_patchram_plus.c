...
/*
 File was modified 2013-01-21 by Tapani Utriainen : added read timeout to avoid
 hang when bt chip not responding as expected (ex: applying brcm_patchram_plus 
 twice, or on a board without bcm4329)
*/
...
usage(char *argv0)
{
	...
	printf("\t<--timeout=seconds (can be fractional)>\n");
...
parse_cmd_line(int argc, char **argv)
{
	...
	PFI parse[] = { parse_patchram, parse_baudrate,
		...
		parse_scopcm, parse_i2s, parse_no2bytes, parse_tosleep, parse_timeout};
...
read_event(int fd, uchar *buffer)
{
	...
	int count, sel;
        ...
        read_timeout.tv_usec = 1000000.0*(timeout - (double)read_timeout.tv_sec);
        
        while ((sel = select(fd + 1, &read_fds, NULL, NULL, &read_timeout)) > 0) {
                read_timeout.tv_sec = timeout;
                read_timeout.tv_usec = 1000000.0*(timeout - (double)read_timeout.tv_sec);
                count = read(fd, &buffer[i], len);
                FD_ZERO(&read_fds);
                FD_SET(fd, &read_fds);                
                i += count;
                len -= count;
        ...
        if (len > 0 && sel == 0) { if (debug) fprintf(stderr, "Read timeout reading nof_bytes\n"); exit(1); }
        ...
        if (sel == 0 && len > 0) { if (debug) fprintf(stderr, "Read timeout reading %02hx bytes\n", buffer[2]); exit(1); }

	if (debug) {
		count += i;
		fprintf(stderr, "received %d\n", count);
                if (sel < 0) fprintf(stderr, "select() returned error, errno=%i\n", errno);
                else
		dump(buffer, count);
	}
}
